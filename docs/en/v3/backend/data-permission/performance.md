...
To ensure high performance of the data permission system, create appropriate database indexes:
...
## Existing System Optimization Suggestions
...
class Factory
{
    ...
    public function build(Builder $builder, User $user): void
    {
        ...
        // Cache user policy to avoid duplicate queries
...
// Add performance monitoring in the aspect processing:
...
class DataScopeAspect
{
    public function process(ProceedingJoinPoint $proceedingJoinPoint)
    {
        ...
        try {
            ...
            if ($duration > 0.1) {
                Log::warning('Data permission processing is taking too long', [
...
```php
// Batch operation optimization for the existing system
class BatchDataPermissionHelper
{
    public static function loadUsersWithPolicies(array $userIds): Collection
    {
        // Batch preload users and their permission policies
...
return [
    ...
    // Data permission dedicated cache
    'data_permission' => [
        'driver' => 'redis',
        'packer' => Hyperf\Utils\Packer\PhpSerializerPacker::class,
...
```php
// Add performance monitoring in the Factory class
class Factory
{
    public function build(Builder $builder, User $user): void
    {
        if (config('app.debug', false)) {
            ...
            $duration = microtime(true) - $start;
            if ($duration > 0.05) { // Exceeds 50ms, record
...
```php
// Register query listener in the service provider
...
```php
// Add coroutine safety mechanism in the data permission context
...
The optimization focus for the current MineAdmin data permission system should be on:
...
5. **Coroutine safety** - Ensure context safety in the Hyperf coroutine environment
