...
The layout system of MineAdmin 3.0 is a flexible and powerful frontend layout solution that supports multiple layout modes and dynamic switching. Compared to version 2.0, the new layout system adopts a unified architecture design, with all layout logic concentrated in the `src/layouts/index.tsx` file, providing better maintainability and extensibility.
...
package "Layout System" {
  ...
  [Header Component (Header)] as Header
  [Sidebar Container] as Sidebar
  [Main Content Area] as Content
  [Footer Component (Footer)] as Footer
...
- **Applicable Scenarios**: Standard admin management interface
- **Component Structure**: Fixed left menu, right content area

### 2. Mixed Layout
- **Features**: Combination of top menu + left submenu layout
- **Applicable Scenarios**: Complex applications requiring multi-level menu navigation
- **Component Structure**: Top main menu, left submenu for the current category

### 3. Columns Layout
- **Features**: Multi-column menu layout
- **Applicable Scenarios**: Large applications with many menu categories
...
|------------|-------------|-------------|---------------|
| `isMixedLayout()` | `boolean` | Check if current layout is Mixed Layout | `store.isMixedLayout()` |
| `isColumnsLayout()` | `boolean` | Check if current layout is Columns Layout | `store.isColumnsLayout()` |
| `isClassicLayout()` | `boolean` | Check if current layout is Classic Layout | `store.isClassicLayout()` |
| `getFixedAsideState()` | `boolean` | Get whether the sub-sidebar is in fixed state | `store.getFixedAsideState()` |
| `getMenuCollapseState()` | `boolean` | Get whether the menu is collapsed | `store.getMenuCollapseState()` |
| `getMobileState()` | `boolean` | Check if currently in mobile state | `store.getMobileState()` |

::: tip API Source Code Location
...
:root {
  ...
  --mine-g-sub-aside-width: 200px;           /* Sub sidebar expanded width */
  --mine-g-sub-aside-collapse-width: 65px;   /* Sub sidebar collapsed width */
  --mine-g-menu-retract-width: 15px;         /* Menu indentation width */
...
### Layout Transition Animation Optimization
...
- [Common Stores](/v3/front/high/store) - State Management Documentation

::: tip Source Code Reference
The complete layout system source code can be found at:
