...
Demonstrates different configurations of operation columns, including conditional display, custom styles, and complex operation logic.

<DemoPreview dir="demos/ma-pro-table-examples/custom-operations" />

## Features

- **Multiple Operation Types**: Supports tile, dropdown, auto-collapse display modes
...
```javascript
{
  ...
  operationConfigure: {
    ...
    fold: 2,                // Collapse count in auto mode
    actions: [
      {
        name: 'view',
        text: 'Details',
...
operationConfigure: {
  type: 'auto',
  fold: 2,                  // Collapse after displaying 2 buttons
  actions: [...]
}
```
...
const batchDelete = () => {
  ElMessageBox.confirm('Confirm batch deletion of selected items?', 'Batch Operation', {
    type: 'warning'
  }).then(() => {
...
The custom operations feature allows you to build complex interaction logic to meet the operational requirements of various business scenarios.