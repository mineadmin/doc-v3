...
Demonstrates how to extend toolbar functionality through the plugin API, including custom tool buttons and toolbar layout.

<DemoPreview dir="demos/ma-pro-table-examples/toolbar-extensions" />

## Features

- **Plugin Mechanism**: Extend toolbar functionality through the plugin API
- **Custom Tools**: Support for registering custom tool buttons
- **Slot Extensions**: Support for extending toolbar layout through slots
...
add({
  ...
  render: ({ proxy }) => (
    <el-button
      ...
      title="Import Data"
      onClick={() => {
        ElMessageBox.confirm('Are you sure you want to import data?', 'Import Confirmation', {
          type: 'warning'
        }).then(() => {
          // Trigger file selection or display import dialog
  ...
  render: ({ proxy }) => (
    <el-button
      ...
      onClick={async () => {
        ...
        await proxy.refresh()
        ElMessage.success('Refresh complete')
      }}
    >
    ...
    <template #toolbarLeft>
      <div class="custom-toolbar-left">
        <el-space>
          ...
          <el-text type="success">Active: {{ activeCount }}</el-text>
        </el-space>
      </div>
    </template>
  </MaProTable>
...
<template>
  <MaProTable :options="options" :schema="schema">
    <!-- Toolbar prepend content -->
    <template #beforeToolbar>
      <el-button size="small" type="text">Prepend Button</el-button>
    </template>
    
    <!-- Toolbar append content -->
    <template #afterToolbar>
      <el-button size="small" type="text">Append Button</el-button>
    </template>
  </MaProTable>
</template>
```
...
### Configure Tool Display State
```javascript
const options = {
  toolbar: true,                // Whether to display the toolbar
  toolStates: {
...
### Dynamically Control Tool State
```javascript
// Runtime control
  ...
  render: ({ proxy }) => (
    <el-button
      ...
      onClick={() => {
        ...
        ElNotification({
          title: 'System Monitor',
          message: `CPU: ${status.cpu}% | Memory: ${status.memory}% | Database: ${status.db}`,
          type: 'warning',
          duration: 4000
        })
      }}
...
The toolbar extension feature allows you to build feature-rich table operation interfaces, enhancing the user's operational experience.