...
Demonstrates complete CRUD operations, including add, edit, delete, and data linking functionalities.

<DemoPreview dir="demos/ma-pro-table-examples/data-management" />

## Features

- **Complete CRUD**: Supports data create, read, update, and delete operations
- **Form Linking**: Add/edit forms are linked with table data
- **Data Statistics**: Real-time statistical data indicators
- **Batch Operations**: Supports batch selection and operations
- **State Synchronization**: Automatically refreshes table data after operations
  ...
  <el-card shadow="never">
    <template #header>
      <div class="card-header">
        <span>Data Operations Panel</span>
        <el-space>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="Name" prop="name">
            <el-input v-model="employeeForm.name" placeholder="Please enter name" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
const formRules = {
  name: [
    ...
    { min: 2, max: 10, message: 'Name length should be 2 to 10 characters', trigger: 'blur' }
  ],
  department: [
...
const handleSaveEmployee = async () => {
  ...
  try {
    ...
    if (editingEmployee.value) {
      ...
      ElMessage.success('Employee info updated successfully')
    } else {
      // Add mode
...
const handleDeleteEmployee = (employee) => {
  ElMessageBox.confirm(`Are you sure you want to delete employee "${employee.name}"?`, 'Delete Confirmation', {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`Are you sure you want to delete the selected ${selectedRows.length} records?`, 'Batch Delete', {
    confirmButtonText: 'Confirm',
    cancelButtonText: 'Cancel',
    type: 'warning'
  }).then(() => {
...
const handleSearchSubmit = (form) => {
  ...
  ElMessage.info('Executing search operation')
...
const handleSearchReset = (form) => {
  ...
  ElMessage.info('Resetting search criteria')
  return form
}
```
...
const refreshTableData = async () => {
  try {
    await tableRef.value?.refresh()
    ElMessage.success('Data refresh successful')
  } catch (error) {
...
const validateFormData = (data) => {
  ...
  if (data.salary < 3000 || data.salary > 100000) {
    errors.push('Salary range is invalid')
  }
...
- Properly use debounce and throttle

The data management functionality allows you to build complete data operation interfaces, meeting data management needs for various business scenarios.