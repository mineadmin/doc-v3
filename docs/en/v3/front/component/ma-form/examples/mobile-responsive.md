...
Showcases the responsive features of MaForm on mobile devices, including breakpoint adaptation, mobile optimization, touch interaction, and device-specific configuration.

<DemoPreview dir="demos/ma-form/mobile-responsive" />

## Features

- **Breakpoint Responsive**: Auto-adapt layout based on screen size
- **Mobile Optimization**: Dedicated optimizations for mobile devices
- **Touch-Friendly**: Interaction design adapted for touch operations
- **Device Detection**: Auto-identifies mobile devices and applies corresponding configurations
...
const responsiveFormItems = [
  {
    ...
    cols: {
      ...
      xs: 24,    // Extra small screen: occupies full row
      sm: 24,    // Small screen: occupies full row
      md: 12,    // Medium screen: occupies half
      lg: 8,     // Large screen: occupies one third
      xl: 6      // Extra large screen: occupies one quarter
    }
  },
  {
    label: 'Email Address',
  ...
  },
  {
    ...
    mobileProps: {
      type: 'tel',           // Use phone keypad on mobile
...
const mobileSizedFields = [
  {
    ...
    render: 'input',
    renderProps: {
      size: 'default'  // Desktop default size
    },
    mobileProps: {
      size: 'large'    // Large size on mobile for easier touch
    }
  },
  {
    label: 'Selector',
...
const touchFriendlyFields = [
  {
    ...
    mobileProps: {
      showInput: false,
      height: 6,        // Increase slider height for easier touch
      showTooltip: true,
  ...
  },
  {
    ...
    renderProps: {
      max: 5,
      allowHalf: false  // Disable half-star on mobile
    },
    mobileProps: {
      allowHalf: false,
  ...
  },
  {
    ...
    mobileProps: {
      size: 'large',    // Increase switch size for easier clicking
      width: 60
    }
  }
]
...
const mobileUploadField = {
  ...
  mobileProps: {
    ...
    showFileList: true,
    accept: 'image/*',       // Access camera/gallery
  ...
  renderSlots: {
    trigger: () => {
      ...
      }, isMobile ? 'Take/Select Photo' : 'Select File')
    }
  }
}
```
...
// Adapt form based on device characteristics
...
const pwaFormConfig = {
  ...
  loadingConfig: {
    ...
    fullscreen: true,  // Use full screen loading in PWA
    lock: true
  }
}

// PWA offline state handling
        ...
        formRef.value?.updateItem(item.prop, {
          ...
          itemProps: {
            ...
            rules: [
              ...
              { required: true, message: 'Please fill in this field in offline mode', trigger: 'blur' }
            ]
          }
        })
      }
...
const mobilePerformanceOptimization = {
  ...
  enableVirtualScrolling: (threshold = 20) => {
    ...
    if (items.length > threshold) {
      ...
      console.log('Enabled virtual scrolling optimization')
    }
  },
...
- [Layout System - Responsive Configuration](/v3/front/component/ma-form/examples/layout-systems#responsive-system)
- [ResponsiveConfig Configuration](/v3/front/component/ma-form#responsiveconfig)
- [Mobile Configuration Options](/v3/front/component/ma-form#mobile-configuration)