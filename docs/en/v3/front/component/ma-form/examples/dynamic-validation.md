...
Demonstrates MaForm's dynamic validation features, including custom synchronous validation, asynchronous validation, linked validation, and complex validation rules.
...
- **Asynchronous Validation**: Supports server-side validation (uniqueness checks, etc.)
- **Custom Error Messages**: Flexible error prompt customization
...
```typescript
{
  ...
  itemProps: {
    rules: [
      ...
      { min: 3, max: 15, message: 'Length must be between 3 and 15 characters', trigger: 'blur' },
...
```typescript
{
  ...
  asyncValidator: async (rule, value) => {
    ...
    // Asynchronously check if email already exists
    try {
...
const linkedValidationFields = [
  {
    ...
    customValidator: (rule, value, callback) => {
      ...
      if (endDate && new Date(value) >= new Date(endDate)) {
        ...
        callback()
        // Trigger revalidation of end date
        nextTick(() => {
          if (endDate) {
  ...
  },
  {
    ...
    customValidator: (rule, value, callback) => {
      ...
      if (startDate && new Date(value) <= new Date(startDate)) {
        ...
        callback()
        // Trigger revalidation of start date
        nextTick(() => {
          if (startDate) {
...
```typescript
{
  ...
  render: 'input',
  customValidator: (rule, value, callback) => {
    // Determine required status based on user type
    const userType = formData.userType
    const isRequired = userType === 'individual'
    
    if (isRequired && !value) {
      callback(new Error('Individual users must provide a phone number'))
      return
    }
...
```typescript
{
  ...
  customValidator: (rule, value, callback) => {
    ...
    // Business rule: Promotional product price cannot be higher than 1000
    const isPromotional = formData.isPromotional  
    if (isPromotional && value > 1000) {
      callback(new Error('Promotional product price cannot be higher than 1000'))
      return
    }
...
### 1. Trigger Method Configuration
...
- [MaFormItem Validation Configuration](/v3/front/component/ma-form#validation-configuration)
- [Advanced Features - Dynamic Validation](/v3/front/component/ma-form#dynamic-validation)
- [Exposed Methods - Form Validation](/v3/front/component/ma-form#form-validation)