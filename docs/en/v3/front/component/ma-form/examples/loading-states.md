...
- **Dynamic loading state**: Supports runtime dynamic toggling of loading states
- **Loading overlay configuration**: Supports background overlay, scroll locking, etc.
- **Loading slot support**: Supports fully customized loading content
...
### 2. Loading Configuration for Different Scenarios
...
// Loading state for a single field
{
  ...
  renderProps: {
    ...
    placeholder: 'Input for async validation'
  },
...
const submitForm = async () => {
  ...
  try {
    ...
    ElMessage.error('Submission failed, please retry')
  } finally {
    formRef.value.setLoadingState(false)
  }
}
...
const handleLoadingWithTimeout = async (asyncTask: () => Promise<any>, timeout = 30000) => {
  ...
  try {
    ...
    if (error.message === 'Operation timed out') {
      ElMessage.error('Operation timed out, please check network connection')
    } else {
      ElMessage.error('Operation failed')
    }
  } finally {
...
- [LoadingConfig Detailed Configuration](/v3/front/component/ma-form#loadingconfig-configuration)
- [State Management Methods](/v3/front/component/ma-form#state-management)
- [Exposed Methods - Loading State](/v3/front/component/ma-form/examples/expose-methods#state-management-methods)