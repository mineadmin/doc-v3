...
- **Multiple Rendering Methods**: String, component, and function rendering methods
- **Slot Support**: Complete slot system support
- **Props Passing**: Pass component properties via renderProps
- **Event Handling**: Supports all native events of components
  ...
  renderProps: {
    ...
    beforeChange: () => {
      return new Promise((resolve) => {
        ElMessageBox.confirm('Are you sure you want to toggle the status?')
          .then(() => resolve(true))
          .catch(() => resolve(false))
      })
    }
...
```typescript
{
  ...
  renderProps: {
    ...
    texts: ['Terrible', 'Disappointing', 'Average', 'Satisfied', 'Surprising'],
...
```typescript
{
  ...
  renderProps: {
    ...
    beforeUpload: (file) => {
      ...
      if (!isLt2M) {
        ElMessage.error('Uploaded file size cannot exceed 2MB!')
        return false
      }
      return true
    },
  ...
  renderSlots: {
    ...
    tip: () => h('div', { class: 'el-upload__tip' }, 'Only jpg/png files can be uploaded, and must not exceed 2mb')
  }
}
```
  ...
  render: ({ item, model }) => (
    <div class="jsx-field">
      <el-input
        ...
        placeholder="JSX rendering method"
        clearable
        v-slots={{
...
- [Component Rendering System](/v3/front/component/ma-form#component-rendering-system)
- [Supported Element Plus Components](/v3/front/component/ma-form#supported-element-plus-components)
- [Slot System](/v3/front/component/ma-form#slot-system)