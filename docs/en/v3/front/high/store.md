...
MineAdmin uses [Pinia](https://pinia.vuejs.org/) as its state management library, providing a complete state management solution. The system includes multiple commonly used Store modules covering core functionalities such as user management, tabs, plugin system, and dictionary data.

::: tip Auto-import Note
All Stores in the `src/store/modules` directory are configured for automatic import and can be used directly without explicit imports.

**Auto-import configuration location**: The `dirs: ['./src/store/modules/**']` configuration in `vite/auto-import.ts`
:::
...
| `userInfo` | `object \| null` | User basic information |
...
User login method, executes the user authentication flow
...
try {
  ...
  // Login successful, system will redirect automatically
} catch (error) {
...
// Automatically clears: user info, token, page cache, tabs, etc.
```

#### requestUserInfo()
Retrieves user details, including permissions and role data
...
```typescript
// Refresh the current tab (reloads the page component)
...
const navigateToPage = (routeName: string, routeParams?: any) => {
  ...
  // Or specific tab configurations can be added manually
}
```
...
| Property | Type | Description |
|----------|------|-------------|
| `plugins` | `Map<string, Plugin.PluginConfig>` | Loaded plugin configuration mapping |
...
Invokes a specific hook across all enabled plugins

```typescript
// Call login hooks
await pluginStore.callHooks('login', loginFormData)

// Call network request hooks
await pluginStore.callHooks('networkRequest', requestConfig)
```

### Usage Examples

#### Dynamic Plugin State Control
```vue
<template>
...
// Invoke plugin hooks in request interceptor
...
// Invoke plugin hooks in response interceptor
...
Retrieves dictionary data for a given code
...
Retrieves the label for a given dictionary value
...
#### Creating a Dictionary Selection Component
```vue
<!-- DictSelect.vue -->
<template>
...
Besides the core Stores mentioned above, the system also provides other auxiliary Store modules:

### useKeepAliveStore()
Page cache management, see [Frontend Cache System](/v3/front/advanced/cache.md#page-cache-keep-alive)
...
### useSettingStore()
System settings management, including theme, language, layout, and other configurations
...
// ❌ Avoid: Directly destructuring reactive data
const { userInfo, isLogin } = userStore // Will lose reactivity
```

## Related Documentation

- [Auto-import Configuration](/v3/front/advanced/auto-import.md) - Store auto-import mechanism
- [Frontend Cache System](/v3/front/advanced/cache.md) - Page and data caching
- [Plugin System](/v3/front/high/plugins.md) - Plugin development and management
- [Request and Interceptors](/v3/front/advanced/request.md) - Using Store in HTTP requests