...
Language pack files must follow this format: `language identifier[language name].yaml`
...
```typescript
// Smart translation: first global, then local fallback
...
### Checking Translation Key Existence
...
**Issue**: Module or plugin language packs not loading

**Solution**:
1. Ensure filename format is correct: `zh_CN[简体中文].yaml`
2. Check if file path is within scan range
3. Verify YAML syntax is correct

#### 3. Type Errors

**Issue**: TypeScript type checking failures
