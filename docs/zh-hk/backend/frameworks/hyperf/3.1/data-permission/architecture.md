...
MineAdmin 數據權限系統基於 AOP（面向切面編程）設計，通過註解和切面攔截的方式自動為數據查詢注入權限過濾條件。
...
if (策略類型 == DEPT_SELF?) then (是)
    :只獲取當前部門ID;
else (否)
    if (策略類型 == DEPT_TREE?) then (是)
        :獲取當前部門;
        :調用 getFlatChildren();
        :遞歸獲取所有子部門;
        :返回部門ID數組;
    else (否)
        if (策略類型 == CUSTOM_DEPT?) then (是)
            :從策略配置讀取部門ID;
        endif
    endif
endif
...
method -> aspect: 調用帶@DataScope註解的方法
aspect -> aspect: 解析DataScope註解參數
aspect -> factory: 獲取Factory實例
aspect -> builder: 攔截QueryBuilder
factory -> builder: 注入權限過濾條件
...
這個真實的使用示例展示瞭如何在 MineAdmin 中應用數據權限註解來控制用户列表的訪問權限。

通過這個架構設計，MineAdmin 實現了聲明式的數據權限控制，開發者只需要在方法上添加 `@DataScope` 註解，系統就會自動根據當前用户的權限策略過濾數據。