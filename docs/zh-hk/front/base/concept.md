...
新版前端中新增了一個 `./src/plugins` 目錄，專門存放獨立應用或者插件等。插件系統支持功能的熱插拔，提高了系統的可擴展性和靈活性。
...
```plantuml
@startuml
participant "應用主程序" as App
participant "插件管理器" as PM
participant "插件實例" as P
...
- **命名規範**：使用 kebab-case 命名插件目錄
- **版本管理**：在插件配置中明確版本信息
- **依賴聲明**：清晰聲明插件依賴關係
- **資源清理**：實現 uninstall 方法進行資源清理
- **類型支持**：為插件提供完整的 TypeScript 類型定義
- **文檔完善**：為每個插件提供使用文檔和示例
