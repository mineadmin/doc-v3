...
展示 MaForm 在移動端的響應式特性，包括斷點適配、移動端優化、觸摸交互和設備特定配置。
...
const mobileOptimizedFields = [
  ...
  },
  {
    ...
    mobileProps: {
      inputmode: 'email',
      autocomplete: 'email',
      autocapitalize: 'none',  // 禁用自動首字母大寫
      spellcheck: false,       // 禁用拼寫檢查
      size: 'large'
    }
  },
  {
...
const mobileSizedFields = [
  {
    ...
    render: 'input',
    renderProps: {
      size: 'default'  // 桌面端默認尺寸
    },
    mobileProps: {
      size: 'large'    // 移動端大尺寸，便於觸摸
    }
  },
  {
    label: '選擇器',
...
const mobileUploadField = {
  ...
  mobileProps: {
    ...
    drag: false,             // 禁用拖拽
    showFileList: true,
    accept: 'image/*',       // 調用相機/相冊
    capture: 'environment'   // 指定相機類型
  },
  renderSlots: {
    trigger: () => {
      const isMobile = window.innerWidth < 768
        ...
        formRef.value?.updateItem(item.prop, {
          ...
          itemProps: {
            ...
            rules: [
              ...
              { required: true, message: '離線模式下請填寫此字段', trigger: 'blur' }
            ]
          }
        })
      }
...
- [佈局系統 - 響應式配置](/zh-hk/front/component/ma-form/examples/layout-systems#響應式系統)
- [ResponsiveConfig 配置](/zh-hk/front/component/ma-form#responsiveconfig-響應式配置)
- [移動端配置選項](/zh-hk/front/component/ma-form#移動端配置)