...
如果需要傳入其他配置，例如: `header`，`timeout` 等參數，只需要在後面加參數傳入即可。

### token刷新
`useHttp()` 內部已經封裝了自動續期 `token` 功能，具體原理可查看 [後端文檔](/zh-hk/backend/security/passport.md) 這裏不再贅述。
...
`MineAdmin` 已經封裝了攔截器，但需要説明只適配 `MineAdmin` 的後端接口，如果需要調整、修改，打開 `src/utils/http.ts` 文件
找到下面這段代碼：
```ts:line-numbers
