...
MineAdmin 3.0 的佈局系統是一個靈活且強大的前端佈局解決方案，支持多種佈局模式和動態切換。相比 2.0 版本，新的佈局系統採用了統一的架構設計，所有佈局邏輯都集中在 `src/layouts/index.tsx` 文件中，提供了更好的維護性和擴展性。
...
package "佈局系統" {
  ...
  [頭部組件 (Header)] as Header
  [側邊欄容器] as Sidebar
  [主內容區] as Content
  [底部組件 (Footer)] as Footer
  ...
  package "狀態管理" {
    [設置存儲 (useSettingStore)] as SettingStore
    [佈局狀態] as LayoutState
...
- **特點**: 頂部菜單 + 左側子菜單的組合佈局
- **適用場景**: 需要多級菜單導航的複雜應用
- **組件結構**: 頂部主菜單，左側當前分類的子菜單

### 3. 分欄佈局 (Columns Layout)
- **特點**: 多欄式菜單佈局
- **適用場景**: 菜單分類較多的大型應用
- **組件結構**: 左側主菜單欄，中間子菜單欄，右側內容區
...
| `getMenuCollapseState()` | `boolean` | 獲取菜單是否為摺疊狀態 | `store.getMenuCollapseState()` |
...
:root {
  ...
  --mine-g-menu-retract-width: 15px;         /* 菜單縮進寬度 */
...
- [常用 Store](/zh-hk/front/high/store) - 狀態管理相關文檔
