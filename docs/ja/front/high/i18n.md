...
MineAdmin フロントエンドは Vue i18n v11 を基に構築された完全な国際化ソリューションを提供し、多言語切り替え、動的言語パック読み込み、モジュール化翻訳管理をサポートしています。このドキュメントでは、システムの国際化実装メカニズムと使用方法を詳しく説明します。
...
言語パックファイルは特定のフォーマットに従う必要があります：`言語識別子[その言語の名称].yaml`
...
const localTitle = localT('welcome')   // コンポーネント内定義の翻訳
</script>
```
...
const loadLanguage = async (locale: string) => {
  try {
    ...
    console.error(`言語パックの読み込みに失敗: ${locale}`, error)
  }
}
```
