...
MineAdmin フロントエンドは Vue i18n v11 をベースに構築された完全な国際化ソリューションを提供し、多言語切り替え、動的言語パックのロード、モジュール化された翻訳管理をサポートしています。このドキュメントでは、システムの国際化実装メカニズムと使用方法を詳しく説明します。
...
zh_CN.yaml         # 言語名が不足
en[英文].yaml      # 言語名が誤り
zh-CN[簡体字中国語].yaml  # 言語識別子フォーマット誤り
```

### グローバル言語パック

システムは以下の3箇所の言語パックを自動スキャンします：
...
**コア言語パック例** (`src/locales/zh_CN[簡体字中国語].yaml`)：
...
```typescript
// インテリジェント翻訳：グローバル優先、ローカルにフォールバック
...
if (te('some.key')) {
  ...
  console.warn('翻訳キーが見つかりません: some.key')
}
```
...
const loadLanguage = async (locale: string) => {
  try {
    ...
    console.error(`言語パックのロードに失敗しました: ${locale}`, error)
  }
}
```
...
**問題**: モジュールやプラグインの言語パックがロードされない

**解決策**:
1. ファイル名フォーマットが正しいことを確認：`zh_CN[簡体字中国語].yaml`
2. ファイルパスがスキャン範囲内にあることを確認
3. YAML 構文が正しいことを確認

#### 3. 型エラー

**問題**: TypeScript 型チェックが失敗する
