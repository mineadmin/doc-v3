...
すべてのプログラムによる管理方法の使用を展示し、検索項目の追加、削除、変更などの操作を含み、業務ロジックに基づいて検索条件を動的に調整する必要があるシナリオに適用されます。
...
const templates = {
  ...
  orderSearch: [
    ...
    { label: '注文状態', prop: 'order_status', render: 'select', options: orderStatusOptions },
    { label: '注文時間', prop: 'created_at', render: 'date-picker' }
  ]
}
...
### 1. 状態管理
複雑な検索項目状態を管理するために状態管理ライブラリ（Piniaなど）を使用することを推奨：

```typescript
// Pinia storeを使用して検索項目を管理
...
- [メソッドデモ](./methods-demo) - すべての公開メソッドの詳細な使用を理解
- [カスタム操作](./custom-actions) - カスタム操作ボタンを理解
