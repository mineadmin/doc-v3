# MaProTable
`ma-pro-table` は `ma-search` と `ma-table` の2つのコンポーネントを基に構築され、完全な `CRUD` 機能を迅速に構築し、サボる時間を増やすために設計されています。

:::tip ヒント
システムに組み込まれている **ユーザー、ロール管理** はすべて `ma-pro-table` CRUDのベストプラクティスです。ドキュメントと実際の例を参照することで、このコンポーネントを迅速に習得できます。
...
まず、セルのレンダリング内容が異なることは非常に頻繁に発生するシナリオです。多くのコードは同じかもしれませんが、パラメータが異なり、フィールド名が異なるだけです。そのため、`ma-pro-table`を構築する際にこの問題を解決する方法を検討しました。

`ma-pro-table`は`url`、`image`、`video`、`switch`レンダリングなど、さまざまなコンテンツのレンダリングを内蔵できますが、問題はニーズが多様であることです。
内蔵機能はビジネスニーズの成長を永遠に満たすことはできませんが、同時にコードの冗長性を避けるために、このプラグインメカニズムが生まれました。

皆さんは自分がよく使う、または特定のビジネスに特化したセルレンダリングプラグインをアプリケーション市場で共有し、セルレンダリングを豊富にすることができます。これにより、同じものを再び書く必要がなくなります。
:::
...
::: details `MaProTableRenderPlugin` タイプ説明をクリックして表示
| パラメータ       | 説明                  | タイプ         |
|----------|---------------------|-------------------|
...
// プラグインを登録
addPlugin({
  // プラグイン名、一意の識別子。アプリケーション市場にアップロードする場合は、専用のプレフィックスを付けてください
  name: 'tag',
...
::: tip 説明
`ma-pro-table`には[スロット](#slot)もあり、ここを拡張できます。特定の一時的な機能にはスロットを使用し、システム全体に必要な機能がある場合は`api`で拡張することをお勧めします。
:::
...
::: details `MaProTableToolbar` タイプ説明をクリックして表示
| パラメータ       | 説明                  | タイプ         |
|----------|---------------------|-------------------|
...
| `actionBtnPosition`    | アクションボタンの配置位置、自動モードではタイトルバーが有効な場合にタイトルバーに表示され、それ以外の場合はテーブルの左上に表示されます | `auto, header, table`                       | `auto` | 1.0.0  |
...
| パラメータ                | 説明        | タイプ                                                                                 | デフォルト値     | バージョン    |
|-------------------|-----------|------------------------------------------------------------------------------------|---------|-------|
| `enabled`         | 行の右クリックメニューを有効にするかどうか | `boolean`                                                                          | `false` | 1