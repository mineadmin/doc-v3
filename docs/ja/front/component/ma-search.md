...
:::tip ヒント
`form` と `form-item` のパラメータは `ma-form` と同一です。詳細な設定は [ma-form ドキュメント](ma-form) を参照してください。
:::
...
### カスタム拡張
- **[カスタムアクション](./ma-search/examples/custom-actions)** - カスタムボタンとスロット使用
- **[動的管理](./ma-search/examples/dynamic-items)** - 実行時動的追加・削除検索項目
- **[メソッドデモ](./ma-search/examples/methods-demo)** - 公開メソッドの詳細使用法
...
| `searchItems` | 検索フォーム項目設定（[ma-form-item](ma-form#maformitem) 拡張） | `MaSearchItem[]` | - | 1.0.0 |
...
| `cols` | レスポンシブ列数設定（異なる画面サイズ対応） | `MediaBreakPoint` | `{xs: 1, sm: 2, md: 2, lg: 3, xl: 4}` | 1.0.0 |
| `fold` | 折りたたみ機能有効化 | `boolean` | `false` | 1.0.0 |
| `foldRows` | 折りたたみ後の表示行数 | `number` | `2` | 1.0.0 |
| `show` | 検索パネル表示有無 | `boolean` | `true` | 1.0.0 |
| `text` | ボタンテキスト設定 | `TextConfig` | - | 1.0.0 |

#### MediaBreakPoint

レスポンシブブレークポイント設定（異なる画面サイズでの列数定義）：

| パラメータ | 説明 | 画面サイズ | タイプ | デフォルト値 | バージョン |
|------|------|----------|------|-------|------|
| `xs` | 超小型画面表示列数 | `< 768px` | `number` | `1` | 1.0.0 |
| `sm` | 小型画面表示列数 | `≥ 768px` | `number` | `2` | 1.0.0 |
| `md` | 中型画面表示列数 | `≥ 992px` | `number` | `2` | 1.0.0 |
| `lg` | 大型画面表示列数 | `≥ 1200px` | `number` | `3` | 1.0.0 |
| `xl` | 超大型画面表示列数 | `≥ 1920px` | `number` | `4` | 1.0.0 |
...
検索フォーム項目設定（`ma-form-item` 拡張）：

| パラメータ | 説明 | タイプ | デフォルト値 | バージョン |
|------|------|------|-------|------|
| `label` | ラベルテキスト | `string` | - | 1.0.0 |
| `prop` | フィールド名（フォームデータのキー名対応） | `string` | - | 1.0.0 |
...
| `offset` | グリッド左側間隔数 | `number` | `0` | 1.0.0 |
...
| `'date-picker'` | 日付選択器 | `render: 'date-picker'` |
...
| 名前 | 説明 | パラメータ | バージョン |
|------|------|------|------|
| `search` | 検索ボタンクリック時発火 | `(formData: Record<string, any>) => void` | 1.0.0 |
| `reset` | リセットボタンクリック時発火 | `(formData: Record<string, any>) => void` | 1.0.0 |
| `fold` | 折りたたみ状態変更時発火 | `(state: boolean) => void` | 1.0.0 |

### Slots

| 名前 | 説明 | パラメータ | バージョン |
|------|------|------|------|
| `default` | デフォルトスロット（`<el-form-item>` 直接記述可能、設定方式自動無効化） | - | 1.0.0 |
| `actions` | アクションボタン領域完全置換 | `{ searchLoading: boolean, resetLoading: boolean }` | 1.0.0 |
| `beforeActions` | アクションボタン前挿入コンテンツ | - | 1.0.0 |
| `afterActions` | アクションボタン後追加コンテンツ | - | 1.0.0 |
...
| `setItems(items)` | 検索項目設定動的設定 | `items: MaSearchItem[]` | - | 1.0.0 |
| `getItems()` | 現在の検索項目設定取得 | - | `MaSearchItem[]` | 1.0.0 |
| `appendItem(item)` | 単一検索項目追加 | `item: MaSearchItem` | - | 1.0.0 |
...
const searchItems = [
  {
    ...
    rules: [
      ...
      { type: 'email', message: 'メールアドレス形式が不正です', trigger: 'blur' }
    ]
  }
]
```
...
```typescript
{
  ...
  prop: 'conditional',
  render: 'input',
  hide: () => someCondition // true の場合非表示
}
```
