...
MineAdminは、ロールベースアクセス制御（RBAC）システムを採用し、JWT認証、多層権限検証、データレベル権限制御を組み合わせることで、エンタープライズアプリケーションに包括的なセキュリティを提供します。
...
コントローラーメソッドで`@Permission`アノテーションを使用した権限制御：
...
public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
{
    ...
    try {
        ...
        if ($jwt->isBlacklisted($token)) {
            throw new TokenValidException('Tokenがブラックリストに登録されています');
        }
