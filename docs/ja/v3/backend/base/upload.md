...
MineAdmin は、デフォルトのサーバー側ファイルアップロードロジックを提供します。エンドポイントは `/admin/attachment/upload` です。
アップロード成功後、リソースマネージャーに統合されます。
...
### ローカルストレージをOSSストレージに置き換える

一般的なビジネスシナリオでは、ファイルは通常OSSに保存されます。その場合、デフォルトのファイルアップロード処理を置き換える必要があります。例として Alibaba Cloud を挙げます。
まず、`config/autoload/file.php` ファイルを設定し、Alibaba Cloud チャネルを追加します。
次に、新しい `AliyunUploadSubscribe` を作成し、`config/autoload/listeners.php` のデフォルトの `UploadSubscribe` を Alibaba Cloud チャネルに置き換えます。
...
return [
    ...
    'storage' => [
        ...
        'oss' => [
            ...
            'schema' => 'http://',
            'isCName' => false,
            // OSS ドメインアドレス、入力しないとアクセスパス生成に失敗します
            'public_url' => env('APP_URL') . '/uploads',
            // 'timeout'        => 3600,
            // 'connectTimeout' => 10,
            // 'token'          => '',
...
### デフォルトのアップロードファイル名とディレクトリ名を変更する

ファイル名とファイルのディレクトリ名は、デフォルトでは `Mine\Upload\Listener\UploadListener->generatorPath()` と
`Mine\Upload\Listener\UploadListener->generatorId()` によって実装されています。すべてのアップロード処理クラスはこのクラスを継承しています。
上記のOSSストレージへの置き換えを例にとると、アップロード処理クラス内でこれら2つのメソッドを置き換えるだけで済みます。
...
サーバー側のファイルコントローラーがファイルサービスを呼び出し、フロントエンドから渡された `file` を処理します。
`ファイルサービス`は、アップロードされたファイルのハッシュ値が既にアップロードされているかどうかを確認します。
アップロード済みの場合、データベースを照会し、前回のアップロード情報を返します。
未アップロードの場合、`UploadInterface` インスタンスを呼び出して `UploadEvent` イベントを配信します。
配信後、`UploadEvent->isUploaded()` でアップロードが成功したかどうかを判断します。
アップロードが成功した場合、`Upload` アップロードインスタンスを返します。
アップロードが成功しなかった場合、例外をスローしてアップロード失敗とします。
...
|ファイルサービス|
:アップロードされたファイルのハッシュ値が既にアップロードされているか確認;
if (アップロード済み) then (はい)
    :データベースを照会し、前回のアップロード情報を返す;
else (いいえ)
    :UploadInterface インスタンスを呼び出し、UploadEvent を配信;
    |UploadEvent|
    :isUploaded() でアップロード成功か判断;
    if (アップロード成功) then (はい)
...
participant "サーバー側ファイルコントローラー" as Controller
...
FileService -> FileService : ファイルのハッシュ値が既にアップロードされているか確認
alt アップロード済み
    FileService -> Database : データベースを照会
    Database -> FileService : 前回のアップロード情報を返す
    FileService -> Controller : 前回のアップロード情報を返す
    Controller -> Frontend : 前回のアップロード情報を返す
else 未アップロード
    FileService -> Uploader : UploadEvent を配信
    Uploader -> Uploader : isUploaded() でアップロード成功か判断
    alt アップロード成功
        ...
        Uploader -> FileService : 例外をスローしてアップロード失敗
        FileService -> Controller : 例外をスローしてアップロード失敗
        Controller -> Frontend : 例外をスローしてアップロード失敗
    end
end
```

## フロントエンドからの OSS 直接アップロード
