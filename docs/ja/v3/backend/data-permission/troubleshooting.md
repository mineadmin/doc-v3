...
**問題の現象**: ユーザーがデータ権限ポリシーを設定したが、クエリ結果にすべてのデータが表示される
...
```php
// 出典: /Users/zhuzhu/project/mineadmin/app/Library/DataPermission/Factory.php:37-39 に基づく
...
#### 1.3 DataScope アノテーション構成を確認

メソッドに正しい DataScope アノテーションがあることを確認:
...
**問題の現象**: データ権限を有効にするとクエリ速度が著しく低下する
...
```php
// 出典: /Users/zhuzhu/project/mineadmin/app/Model/Permission/Department.php の getFlatChildren メソッドの最適化
...
**問題の現象**: 同じユーザーが異なる時間にクエリしたデータが一致しない

#### 4.1 関連キャッシュをクリア

```php
// キャッシュを使用している場合は、適時にクリアする必要がある
...
```php
// データベースからユーザーおよび関連データを強制的に再読み込み
$user = User::find($userId);
...
**問題の現象**: DataScope アノテーションが機能しない

#### 5.1 AOP 構成を確認

```php
// config/autoload/annotations.php で AOP が正しく構成されているか確認
return [
    'scan' => [
        'paths' => [
            BASE_PATH . '/app',
...
```php
// 出典: /Users/zhuzhu/project/mineadmin/app/Library/DataPermission/Aspects/DataScopeAspect.php ファイルの存在確認
...
MineAdmin のデータ権限問題を調査する際は、以下の順序で実施することを推奨します:

1. **スーパー管理者ステータスを確認** - スーパー管理者はすべての権限チェックをバイパス
2. **ポリシー構成を検証** - ユーザーまたは役職に正しい権限ポリシーがあることを確認
3. **アノテーション構成を確認** - メソッドの DataScope アノテーションが正しいことを確認
4. **AOP が有効か確認** - アスペクトがメソッド呼び出しを正常にインターセプトできることを確認
5. **コルーチンコンテキストを確認** - 新しいコルーチンで権限コンテキストを再設定
6. **クエリパフォーマンスを分析** - 必要なデータベースインデックスを追加
7. **キャッシュをクリア** - データ変更後に関連キャッシュを適時にクリア

実際のコードに基づいたこれらの診断方法により、ほとんどのデータ権限関連の問題を効果的に解決できます。