...
MaFormの様々なローディング状態の設定を紹介します。全体ローディング、部分ローディング、カスタムローディングスタイル、動的ローディング制御を含みます。

<DemoPreview dir="demos/ma-form/loading-states" />

## 機能特性

- **マルチレベルローディング制御**：全体と部分のローディング状態をサポート
- **カスタムローディングスタイル**：カスタムローディングアイコン、テキスト、色などをサポート
- **動的ローディング状態**：実行時の動的ローディング状態切り替えをサポート
- **ローディングマスク設定**：背景マスク、スクロールロックなどの設定が可能
- **ローディングスロット対応**：ローディング内容の完全カスタマイズをサポート
...
// 単一フィールドのローディング状態
{
  ...
  render: 'input',
  renderProps: {
    loading: false,  // フィールド単位のローディング状態
...
### 2. コンポーネントグループのローディング

```typescript
// 関連フィールドグループのローディング状態を設定
...
const submitForm = async () => {
  ...
  try {
    ...
    ElMessage.error('送信失敗、再試行してください')
  } finally {
    formRef.value.setLoadingState(false)
  }
}
...
const getResponsiveLoadingConfig = () => {
  ...
  return {
    ...
    fullscreen: isMobile,  // モバイル端末では全画面ローディング
    lock: true
  }
}
...
- [LoadingConfig 設定詳細](/v3/front/component/ma-form#loadingconfig-設定)
- [状態管理メソッド](/v3/front/component/ma-form#状態管理)
- [公開メソッド - ローディング状態](/v3/front/component/ma-form/examples/expose-methods#状態管理メソッド)