...
MaFormがサポートするすべてのElement Plusコンポーネントのレンダリング方法を紹介します。入力系、選択系、日付系、特殊コンポーネントを含みます。

<DemoPreview dir="demos/ma-form/component-rendering" />

## 機能特性

- **全面サポート**：すべてのElement Plusフォームコンポーネントをサポート
- **複数のレンダリング方法**：文字列、コンポーネント、関数の3種類のレンダリング方法
- **スロットサポート**：完全なスロットシステムをサポート
- **プロパティの受け渡し**：renderPropsを介してコンポーネントのプロパティを渡す
  ...
  renderProps: {
    ...
    beforeChange: () => {
      return new Promise((resolve) => {
        ElMessageBox.confirm('状態を切り替えてもよろしいですか？')
          .then(() => resolve(true))
          .catch(() => resolve(false))
      })
    }
...
```typescript
{
  ...
  renderProps: {
    ...
    beforeUpload: (file) => {
      ...
      if (!isLt2M) {
        ElMessage.error('アップロードファイルのサイズは2MBを超えることはできません！')
        return false
      }
      return true
    },
  ...
  renderSlots: {
    ...
    tip: () => h('div', { class: 'el-upload__tip' }, 'jpg/pngファイルのみアップロード可能、2MB以下に制限')
  }
}
```
...
```typescript
{
  ...
  renderProps: {
    ...
    onFocus: () => {
      console.log('フォーカスを取得')
    },
    onBlur: () => {
      console.log('フォーカスを喪失')
    },
    onClear: () => {
      console.log('選択をクリア')
    }
...
- [サポートされている Element Plus コンポーネント](/v3/front/component/ma-form#サポートされている-element-plus-コンポーネント)
- [スロットシステム](/v3/front/component/ma-form#スロットシステム)