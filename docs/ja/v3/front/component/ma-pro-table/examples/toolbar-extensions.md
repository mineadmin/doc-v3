...
プラグインAPIを介してツールバー機能を拡張する方法を示します。カスタムツールボタンとツールバーレイアウトを含みます。

<DemoPreview dir="demos/ma-pro-table-examples/toolbar-extensions" />

## 機能特徴

- **プラグイン機構**：プラグインAPIを介してツールバー機能を拡張
- **カスタムツール**：カスタムツールボタンの登録をサポート
- **スロット拡張**：スロットを介したツールバーレイアウトの拡張をサポート
- **状態制御**：ツールの動的な表示/非表示をサポート
- **順序制御**：カスタムツールの表示順序をサポート
  ...
  render: ({ proxy }) => (
    <el-button
      ...
      onClick={() => {
        ...
        ElNotification({
          title: 'データ統計',
          message: `合計: ${stats.total} | 在籍中: ${stats.active}`,
          type: 'info'
        })
      }}
    >
...
add({
  ...
  render: ({ proxy }) => (
    <el-button
      ...
      title="データインポート"
      onClick={() => {
        ElMessageBox.confirm('データをインポートしてもよろしいですか？', 'インポート確認', {
          type: 'warning'
        }).then(() => {
          // ファイル選択トリガーまたはインポートダイアログ表示
  ...
  <MaProTable :options="options" :schema="schema">
    <template #toolbarLeft>
      <div class="custom-toolbar-left">
        <el-space>
          <el-text type="primary">全 {{ totalCount }} 件のレコード</el-text>
          <el-divider direction="vertical" />
          <el-text type="success">在籍中 {{ activeCount }} 人</el-text>
        </el-space>
      </div>
    </template>
  </MaProTable>
...
<template>
  <MaProTable :options="options" :schema="schema">
    ...
    <template #beforeToolbar>
      <el-button size="small" type="text">前置ボタン</el-button>
    </template>
    
    <!-- ツールバー後置コンテンツ -->
    <template #afterToolbar>
      <el-button size="small" type="text">後置ボタン</el-button>
    </template>
  </MaProTable>
</template>
```
