...
Laravel 实现是 MineAdmin 后端多实现方案的预留入口。第一阶段只建立文档结构，不补齐完整实现细节。
...
当前页面只作为规划入口。正式使用 Laravel 实现前，请以 [Hyperf 实现](/v3/backend/frameworks/hyperf/) 和公共契约为准。
