...
2. [Hyperf 实现](/v3/backend/frameworks/hyperf/)：当前 v3 稳定实现，包含生命周期、中间件、异常处理、日志、事件、上传、多语言、认证授权和数据权限等细节。
...
- [Hyperf 用户认证](/v3/backend/frameworks/hyperf/security/passport)：双 Token、JWT 与登录流程。
- [Hyperf 用户授权（RBAC）](/v3/backend/frameworks/hyperf/security/access)：角色、菜单、权限校验与审计。
- [Hyperf 数据权限](/v3/backend/frameworks/hyperf/data-permission/overview)：部门、岗位、策略和数据过滤规则。
