...
展示 MaForm 在真實業務場景中的複雜應用，包括多步驟表單、資料字典整合、許可權控制、國際化等高階特性。
  ...
  return {
    ...
    async onMounted() {
      try {
        ...
        console.error(`載入字典資料失敗: ${dictConfig.code}`, error)
        formRef.value?.updateItem(prop, {
          renderProps: { loading: false }
        })
      }
  ...
  updateFieldPermission: (fieldProp: string, newPermission: Partial<FieldPermission>) => {
    ...
    if (permission) {
      ...
      formRef.value?.updateItem(fieldProp, {
        ...
        itemProps: {
          rules: required ? [
            { required: true, message: '此欄位為必填項', trigger: 'blur' }
          ] : []
        }
      })
    }
...
const i18nService = {
  ...
  messages: ref<Record<string, Record<string, string>>>({
    'zh-CN': {
      ...
      'form.reset': '重置',
      'form.cancel': '取消',
      'field.required': '此欄位為必填項',
      'field.username': '使用者名稱',
      'field.email': '郵箱',
      'field.phone': '電話',
      'placeholder.username': '請輸入使用者名稱',
...
const registerBusinessRules = () => {
  ...
  businessRuleEngine.registerRule({
    ...
    name: 'VIP客戶欄位顯示',
    description: '當客戶型別為VIP時，顯示VIP專屬欄位',
            ...
            rules: [
              { 
                validator: (rule, value, callback) => {
                  if (value > limit) {
                    callback(new Error(`${customerLevel}客戶單筆交易限額為${limit}元`))
                  } else {
                    callback()
                  }
                },
...
const dataLinkageManager = {
  ...
  linkageConfig: new Map<string, {
    triggers: string[]                    // 觸發欄位
    target: string                        // 目標欄位
    handler: (data: any) => any | Promise<any>  // 處理函式
...
- [條件渲染](/zh-tw/front/component/ma-form/examples/conditional-rendering)
- [動態驗證](/zh-tw/front/component/ma-form/examples/dynamic-validation)
- [巢狀表單](/zh-tw/front/component/ma-form/examples/nested-forms)
- [暴露方法](/zh-tw/front/component/ma-form/examples/expose-methods)