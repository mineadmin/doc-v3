...
本文件提供完整的 MineAdmin 外掛開發示例，包括不同型別外掛的實際程式碼案例和最佳實踐。
...
App-Store 是 MineAdmin 唯一的官方預設外掛，提供應用市場管理功能，展示了混合型外掛的完整實現。
...
以下是一個完整的使用者管理外掛示例，展示如何開發一個包含前後端的混合型外掛。
...
class UserController extends MineController
{
    ...
    public function create(): array
    {
        ...
        return $this->success($user, '使用者建立成功');
    }
...
- **外掛名稱**: 使用 `vendor/plugin-name` 格式
- **名稱空間**: `Plugin\VendorName\PluginName`
- **類名**: 使用大駝峰命名法
- **方法名**: 使用小駝峰命名法
...
class ExampleController extends MineController
{
    ...
    public function create(): array
    {
        ...
        return $this->success($result, '建立成功');
    }
        $user = $this->userService->find($id);
        
        if (!$user) {
            return $this->error('使用者不存在', 404);
        }
    ...
    public function update(int $id): array
    {
        ...
        return $this->success($user, '使用者更新成功');
    }
    ...
    public function import(): array
    {
        ...
        if (!$file || !$file->isValid()) {
            return $this->error('請上傳有效的檔案');
        }
        
        $result = $this->userService->importFromFile($file);
        
        return $this->success($result, '匯入完成');
    }
    ...
    public function export(): array
    {
        ...
        return $this->success(['file_path' => $filePath], '匯出成功');
    }
}
```
...
#### 5. 資料倉庫 (src/Repository/UserRepository.php)
  ...
  <div class="user-manager">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="last_login_at" label="最後登入" width="160" />
...
const handleToggleStatus = async (row) => {
  ...
  try {
    ...
    row.status_text = newStatus === 1 ? '啟用' : '停用'
  } catch (error) {
...
#### 9. 資料庫遷移 (Database/Migrations/create_users_table.php)
...
class CreateUsersTable extends Migration
{
    public function up(): void
    {
        Schema::create('plugin_user_manager_users', function (Blueprint $table) {
            ...
            $table->tinyInteger('status')->default(1)->comment('狀態:0停用,1啟用');
...
#### 10. 安裝指令碼 (src/InstallScript.php)
