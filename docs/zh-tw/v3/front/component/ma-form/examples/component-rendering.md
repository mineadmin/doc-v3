...
展示 MaForm 支援的所有 Element Plus 元件渲染方式，包括輸入類、選擇類、日期類和特殊元件。

<DemoPreview dir="demos/ma-form/component-rendering" />

## 功能特性

- **全面支援**：支援所有 Element Plus 表單元件
- **多種渲染方式**：字串、元件、函式三種渲染方式
- **插槽支援**：完整的插槽系統支援
- **屬性傳遞**：透過 renderProps 傳遞元件屬性
- **事件處理**：支援元件的所有原生事件
...
```typescript
{
  ...
  renderSlots: {
    default: () => [
      h('el-checkbox', { label: 'reading' }, '閱讀'),
...
```typescript
{
  ...
  renderProps: {
    ...
    beforeUpload: (file) => {
      ...
      if (!isLt2M) {
        ElMessage.error('上傳檔案大小不能超過 2MB!')
        return false
      }
      return true
    },
  ...
  renderSlots: {
    trigger: () => h('el-button', { size: 'small', type: 'primary' }, '選擇檔案'),
    tip: () => h('div', { class: 'el-upload__tip' }, '只能上傳jpg/png檔案，且不超過2mb')
  }
}
```
  ...
  render: ({ item, model }) => (
    <div class="jsx-field">
      <el-input
        ...
        clearable
        v-slots={{
          append: () => <el-button icon="Search">搜尋</el-button>
        }}
      />
    </div>
  )
...
- [元件渲染系統](/v3/front/component/ma-form#元件渲染系統)
- [支援的 Element Plus 元件](/v3/front/component/ma-form#支援的-element-plus-元件)
- [插槽系統](/v3/front/component/ma-form#插槽系統)