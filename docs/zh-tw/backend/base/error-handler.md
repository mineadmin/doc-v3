...
在 `config/autolaod/exceptions.php` 中，預設自帶了一些異常處理器.
`AppExceptionHandler` 是對整個應用程式最終處理措施。如非必要請勿修改順序邏輯
...
也就意味著，當應用發生錯誤時。如果 `APP_DEBUG=true` 則會列印`呼叫棧`以及在命令列輸出`錯誤日誌`
當然我們並不推薦你直接進行 `throw new Exception` 的操作。而是推薦使用 `throw new BusinessException` 丟擲業務異常
以下是預設自帶的業務異常
...
儘管預設自帶的異常處理已經足夠日常開發了，但如果遇到特殊的業務場景需要二次開發自己的異常處理器。那麼就需要
保證你的異常處理器繼承於 `App\Exception\Handler\AbstractHandler`. 並且實現 `handleResponse` 以及
`isValid` 方法

`isValid` 將當前的異常例項傳入進來。需要返回 true 或 false 來判斷當前異常是否需要捕獲，`handleResponse` 則是當捕獲到異常時，則呼叫此方法。並把返回的 Result 例項返回到 client

例如預設提供的 `UnauthorizedExceptionHandler` 異常處理類。他只處理 `UnauthorizedException`。也就是
