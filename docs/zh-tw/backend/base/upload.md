...
檔案上傳由 MineAdmin 接入 [mineadmin/upload](https://github.com/mineadmin/upload) 元件建設而成

:::

MineAdmin 提供了一個預設的服務端檔案上傳邏輯。介面 `/admin/attachment/upload`
並且在上傳成功後接入了資源管理器。
...
在日常的業務場景中，一般檔案儲存在 OSS 上。那麼此時就需要替換預設的檔案上傳處理了。以阿里雲舉例
首先我們要配置 `config/autoload/file.php` 檔案。新增阿里雲通道。
然後新建一個 `AliyunUploadSubscribe` 替換 `config/autoload/listeners.php` 中預設的 `UploadSubscribe` 指定為阿里雲通道
...
檔案的命名以及檔案的目錄命名預設是由 `Mine\Upload\Listener\UploadListener->generatorPath()` 和
`Mine\Upload\Listener\UploadListener->generatorId()` 實現的，而所有的上傳處理類都是繼承與這個類。
那麼以上一張替換 OSS 儲存為例。只需要在你的上傳處理類中替換這兩個方法即可
...
`前端`呼叫介面 `/admin/attachment/upload` ，傳入引數 `file`、引數型別為檔案。
服務端檔案控制器呼叫 檔案服務對前端傳入的 `file` 進行處理、
`檔案服務`判斷上傳檔案hash值是否已經上傳過
如果有。則查詢資料庫返回上次上傳的資訊
如果沒有。則`呼叫 UploadInterface 例項分發一個 UploadEvent 事件`
分發後。`判斷 UploadEvent->isUploaded() 是否上傳成功`
如果上傳成功則返回 `Upload` 上傳例項
如果沒有上傳成功則丟擲異常上傳失敗
...
|前端|
start
:呼叫介面 /admin/attachment/upload，傳入檔案引數 file;
|服務端檔案控制器|
:呼叫檔案服務處理 file;
|檔案服務|
:判斷上傳檔案 hash 值是否已上傳過;
if (已上傳過) then (是)
    ...
    :呼叫 UploadInterface 例項分發 UploadEvent;
    |UploadEvent|
...
participant "服務端檔案控制器" as Controller
participant "檔案服務" as FileService
participant "UploadInterface" as Uploader
participant "資料庫" as Database

Frontend -> Controller : 呼叫介面 /admin/attachment/upload，傳入 file
Controller -> FileService : 處理 file
FileService -> FileService : 判斷檔案 hash 值是否已上傳過
alt 已上傳過
    FileService -> Database : 查詢資料庫
    Database -> FileService : 返回上次上傳資訊
    FileService -> Controller : 返回上次上傳資訊
    Controller -> Frontend : 返回上次上傳資訊
else 未上傳過
    FileService -> Uploader : 分發 UploadEvent
    Uploader -> Uploader : 判斷 isUploaded() 是否上傳成功
    alt 上傳成功
        Uploader -> FileService : 返回 Upload 上傳例項
        FileService -> Controller : 返回 Upload 上傳例項
        Controller -> Frontend : 返回 Upload 上傳例項
    else 上傳失敗
        Uploader -> FileService : 丟擲異常上傳失敗
        FileService -> Controller : 丟擲異常上傳失敗
        Controller -> Frontend : 丟擲異常上傳失敗
    end
end
```
